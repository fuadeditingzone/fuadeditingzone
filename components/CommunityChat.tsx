...
// Added ChatBubbleIcon to imports
...
const AgentProfileModal: React.FC<{ user: ChatUser; onClose: () => void; onMessage: () => void }> = ({ user, onClose, onMessage }) => {
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-[700] bg-black/80 backdrop-blur-md flex items-center justify-center p-6 select-none"
      onClick={onClose}
    >
      <motion.div 
        initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
        className="relative w-full max-w-[500px] glass-modal rounded-[3rem] p-10 md:p-12 shadow-[0_40px_100px_rgba(0,0,0,0.8)] overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        <button onClick={(e) => { e.preventDefault(); onClose(); }} className="absolute top-6 right-6 p-2 text-gray-500 hover:text-white transition-colors">
          <CloseIcon className="w-6 h-6" />
        </button>

        <div className="flex flex-col items-center text-center">
            <div className="relative mb-8">
                <div className={`w-32 h-32 rounded-[2.5rem] overflow-hidden border-4 ${isOwner ? 'border-red-600 shadow-[0_0_30px_rgba(255,0,0,0.3)]' : 'border-white/10'} p-1 bg-black`}>
                    <img src={user.avatar} className="w-full h-full object-cover rounded-[2rem]" alt="" />
                </div>
                <div className={`absolute bottom-1 right-1 w-6 h-6 rounded-full border-4 border-black ${user.online ? 'bg-green-500 shadow-[0_0_15px_rgba(34,197,94,0.4)]' : 'bg-gray-700'}`}></div>
            </div>
            ...
            <div className="w-full space-y-4 mb-10">
                ...
                <div className="text-left p-6 bg-white/5 rounded-2xl border border-white/5">
                    ...
                    <p className="text-[11px] text-gray-400 font-medium leading-relaxed italic">
                        "{profile?.bio || 'No public signal detected for this entity.'}"
                    </p>
                </div>
            </div>

            <button onClick={(e) => { e.preventDefault(); onMessage(); onClose(); }} className="w-full py-5 bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-[0.6em] text-[10px] rounded-2xl shadow-xl transition-all active:scale-95">Initiate Link</button>
        </div>
      </motion.div>
    </motion.div>
  );
        ...
        {showSetup && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/98 backdrop-blur-3xl">
             <div className="w-full max-w-lg bg-[#080808] border border-white/10 rounded-[3rem] p-12 shadow-2xl relative overflow-hidden">
                ...
                <p className="text-gray-500 text-[10px] uppercase font-bold tracking-widest mb-10">Select your sector identity</p>
                <form onSubmit={handleSetup} className="space-y-6">
                   <div>
                      <label className="text-[9px] font-black text-gray-600 uppercase tracking-widest mb-2 block ml-1">Operational Role</label>
          ...
          >
             ...
             >
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-3">
                    ...
                    {searchTerm && users.filter(u => u.id !== clerkUser?.id && (u.name.toLowerCase().includes(searchTerm.toLowerCase()) || u.username.toLowerCase().includes(searchTerm.toLowerCase()))).length === 0 && (
                        <p className="text-center py-10 text-[10px] text-gray-600 uppercase font-black tracking-widest">No matching signatures</p>
                    )}
                </div>
             </motion.div>
          </motion.div>
      ...
      <div className="container mx-auto px-6 max-w-6xl">
        ...
        <div className="w-full bg-[#080808] border border-white/5 rounded-[3.5rem] overflow-hidden shadow-2xl flex flex-col md:flex-row h-[800px] relative">
          ...
          <div className="w-full md:w-80 border-r border-white/5 flex flex-col bg-[#050505]/50">
            <div className="p-8 border-b border-white/5 flex items-center justify-between bg-black/30">
                <div className="flex items-center gap-3">
                    <div className="w-1.5 h-1.5 bg-red-600 rounded-full animate-pulse shadow-[0_0_8px_rgba(255,0,0,0.6)]"></div>
            ...
            <button onClick={(e) => { e.preventDefault(); setIsGlobal(true); setSelectedUser(null); }} className={`flex items-center gap-4 p-7 border-b border-white/5 transition-all ${isGlobal ? 'bg-red-600/10' : 'hover:bg-white/5'}`}>
              ...
              <div className="text-left">
                <p className={`text-[11px] font-black uppercase tracking-widest ${isGlobal ? 'text-white' : 'text-gray-500'}`}>Global Comms</p>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              ...
              {recentChats.length === 0 && (
                <div className="py-20 text-center px-8 opacity-20">
                    <ChatBubbleIcon className="w-10 h-10 mx-auto mb-4" />
                    <p className="text-[9px] font-black uppercase tracking-widest leading-loose">No active private channels detected. Search to find agents.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col bg-black">
            <div className="p-8 border-b border-white/5 flex items-center justify-between bg-black/40 backdrop-blur-xl">
               <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-red-600/15 flex items-center justify-center border border-red-600/30 overflow-hidden shadow-inner">
                    {isGlobal ? <GlobeAltIcon className="w-6 h-6 text-red-500" /> : <img src={selectedUser?.avatar} className="w-full h-full object-cover cursor-pointer" onClick={() => selectedUser && setViewingProfile(selectedUser)} />}
                  </div>
                  <div>
            ...
            <div className="flex-1 overflow-y-auto p-8 md:p-10 space-y-2 custom-scrollbar">
              {messages.map(msg => (
                <div key={msg.id} className={`flex gap-5 mb-5 ${msg.senderId === clerkUser?.id ? 'flex-row-reverse' : 'flex-row'}`}>
                   <img src={msg.senderAvatar} className="w-10 h-10 rounded-[0.8rem] border border-white/5 object-cover cursor-pointer shadow-lg active:scale-90" alt="" onClick={() => { const u = users.find(usr => usr.id === msg.senderId); if(u) setViewingProfile(u); }} />
                   <div className={`max-w-[75%] ${msg.senderId === clerkUser?.id ? 'items-end' : 'items-start'} flex flex-col`}>
                      ...
                      <div className={`p-4 rounded-[1.8rem] text-[13px] font-medium border ${msg.senderId === clerkUser?.id ? 'bg-red-600/10 border-red-600/30 text-white rounded-tr-none' : 'bg-white/5 border-white/10 text-gray-300 rounded-tl-none shadow-lg'}`}>{msg.text}</div>
                   </div>
                </div>
              ))}
                ...
                <div className="space-y-4">
                    {isLocked && (
                        <div className="bg-red-600/10 border border-red-600/20 rounded-[2.5rem] p-8 text-center shadow-[0_0_50px_rgba(255,0,0,0.1)]">
                            <p className="text-[10px] font-black text-red-500 uppercase mb-5 tracking-[0.4em]">Signal Restriction (3/3 Comms)</p>
                            <button onClick={async (e) => { e.preventDefault(); const cid = chatPath!.replace('messages/', ''); await set(ref(db, `connections/${cid}`), { status: 'pending', initiatorId: clerkUser.id }); }} className="w-full py-5 bg-red-600 text-white font-black uppercase text-[10px] tracking-[0.5em] rounded-2xl shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3">
                                <SparklesIcon className="w-4 h-4" /> Send Request to Unlock Comms
                            </button>
                        </div>
                    )}
