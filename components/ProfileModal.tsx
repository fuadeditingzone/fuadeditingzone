...
export const ProfileModal: React.FC<{ isOpen: boolean; onClose: () => void; viewingUserId?: string | null; initialTab?: TabType; forceOpenUpload?: boolean }> = ({ isOpen, onClose, viewingUserId, initialTab = 'identity', forceOpenUpload = false }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setCopyFeedback("Saved Successfully!");
            setTimeout(() => {
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[2000000] flex items-center justify-center p-2 md:p-4">
                    ...
                    <motion.div 
                        ...
                        className="relative w-full max-w-5xl h-full max-h-[92vh] bg-[#050505] rounded-[2rem] md:rounded-[3rem] border border-white/10 flex flex-col overflow-hidden shadow-[0_0_100px_rgba(0,0,0,1)]"
                    >
                        {/* Header */}
                        <div className="flex-shrink-0 p-4 md:p-6 md:px-12 flex items-center bg-black/50 border-b border-white/5 z-20">
                            <button onClick={onClose} className="mr-4 md:mr-6 p-2 rounded-full hover:bg-white/5 transition-all">
                                <ChevronLeftIcon className="w-6 h-6 md:w-7 md:h-7 text-white" />
                            </button>
                            <div className="flex-1 truncate">
                                <h2 className="text-lg md:text-xl font-black text-white uppercase tracking-tight truncate">{targetUser?.username || clerkUser.username}</h2>
                            </div>
                            <button onClick={onClose} className="p-2 rounded-full hover:bg-red-600 text-white transition-all"><CloseIcon className="w-5 h-5 md:w-6 md:h-6" /></button>
                        </div>

                        {/* Scrollable Content */}
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                            <div className="p-6 md:p-12 space-y-8 md:space-y-10 pb-40">
                                            ...
                                            <div className="flex gap-2">
                                                {isViewingOther ? (
                                                    <>
                                                        ...
                                                        <button onClick={() => handleAction('friend')} className="px-5 md:px-6 py-2 bg-white/5 border border-white/10 rounded-xl font-black text-[10px] uppercase tracking-widest text-white transition-all">Add Friend</button>
                                                    </>
                                                ) : (
                                                    <button onClick={() => setActiveTab('identity')} className="px-5 md:px-6 py-2 bg-white/10 hover:bg-white/20 rounded-xl font-black text-[10px] uppercase tracking-widest text-white transition-all">Edit Info</button>
                                                )}
                                            </div>
                                        </div>

                                        <div className="flex justify-center md:justify-start gap-8 md:gap-12">
                                            <div><p className="text-base md:text-lg font-black text-white">{userPosts.length}</p><p className="text-[9px] md:text-[10px] text-zinc-500 uppercase font-black tracking-widest">Posts</p></div>
                                            <div><p className="text-base md:text-lg font-black text-white">{socialState.followers}</p><p className="text-[9px] md:text-[10px] text-zinc-500 uppercase font-black tracking-widest">Followers</p></div>
                                            <div><p className="text-base md:text-lg font-black text-white">{socialState.following}</p><p className="text-[9px] md:text-[10px] text-zinc-500 uppercase font-black tracking-widest">Following</p></div>
                                        </div>
                                ...
                                <div className="flex justify-center border-t border-white/5 pt-4 sticky top-0 bg-[#050505] z-10 overflow-x-auto no-scrollbar">
                                    {(['identity', 'credentials', 'networks', 'posts'] as TabType[]).map(tab => (
                                        <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 md:px-10 py-4 text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] relative whitespace-nowrap ${activeTab === tab ? 'text-white' : 'text-zinc-600 hover:text-zinc-400'}`}>
                                            {tab === 'posts' ? 'Portfolio' : tab === 'credentials' ? 'Skills' : tab}
                                                    ...
                                                    {!showUploadForm ? (
                                                        ...
                                                        >
                                                            <div className="flex justify-between items-center mb-2">
                                                                <span className="text-[10px] font-black text-white uppercase tracking-widest">New Post</span>
                                                            ...
                                                            <div className="flex gap-2">
                                                                ...
                                                                <button 
                                                                    ...
                                                                    className={`flex-1 flex items-center justify-center gap-2 p-4 rounded-xl border transition-all text-[10px] font-black uppercase ${selectedFile ? 'bg-green-600/20 text-green-500' : 'bg-white/5 border-white/10 text-zinc-400 hover:bg-white/10'}`}
                                                                >
                                                                ...
                                                                >
                                                                    {isUploading ? 'Uploading...' : <><SendIcon className="w-3.5 h-3.5" /> Upload Post</>}
                                                                </button>
                                                            </div>
                                                        </motion.div>
                                                    )}
                                    ...
                                    {activeTab === 'credentials' && (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 max-w-2xl mx-auto">
                                            ...
                                            <div className="p-5 md:p-6 bg-white/5 rounded-2xl border border-white/10">
                                                ...
                                                {!isViewingOther ? (
                                                    <input value={profileData.profession} onChange={e => setProfileData({...profileData, profession: e.target.value})} className="bg-transparent text-white font-black w-full outline-none" placeholder="Primary Skills" />
                                                ) : <p className="text-white font-black">{profileData.profession || 'Specialist'}</p>}
                                            </div>
                                            <div className="p-5 md:p-6 bg-white/5 rounded-2xl border border-white/10 md:col-span-2">
                                                <p className="text-[10px] text-zinc-500 font-black uppercase mb-2">Work Experience</p>
                                                {!isViewingOther ? (
                                                    <input value={profileData.experience} onChange={e => setProfileData({...profileData, experience: e.target.value})} className="bg-transparent text-white font-black w-full outline-none" placeholder="Years or Details" />
                                    ...
                                    {activeTab === 'networks' && (
                                        <div className="space-y-4 max-w-2xl mx-auto">
                                            {['instagram', 'facebook', 'tiktok', 'behance', 'twitter'].map(platform => {
                                                ...
                                                return (
                                                    <div key={platform} className="p-5 md:p-6 bg-white/5 rounded-2xl border border-white/10 flex items-center justify-between">
                        ...
                        {!isViewingOther && (activeTab !== 'posts') && (
                            <div className="p-4 md:px-12 border-t border-white/10 bg-black/90 backdrop-blur-md absolute bottom-0 left-0 right-0 z-30">
                                <button 
                                    onClick={handleSave} 
                                    disabled={isSaving}
                                    className="w-full py-4 md:py-5 bg-red-600 text-white font-black uppercase tracking-[0.4em] text-[10px] md:text-[11px] rounded-2xl shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
                                >
                                    {isSaving ? 'Saving...' : copyFeedback ? copyFeedback : 'Save Information'}
