...
export const ProfileModal: React.FC<{ isOpen: boolean; onClose: () => void; viewingUserId?: string | null; initialTab?: TabType; forceOpenUpload?: boolean }> = ({ isOpen, onClose, viewingUserId, initialTab = 'identity', forceOpenUpload = false }) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            console.error(err);
            alert("Upload protocol failed. Ensure Cloudflare Worker is properly bound to R2.");
        } finally {
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[2000000] flex items-center justify-center p-2 md:p-4">
                    ...
                    <motion.div 
                        ...
                        className="relative w-full max-w-5xl h-full max-h-[90vh] bg-[#050505] rounded-[2rem] md:rounded-[3rem] border border-white/10 flex flex-col overflow-hidden shadow-[0_0_100px_rgba(0,0,0,1)]"
                    >
                        {/* Header */}
                                            ...
                                            <div className="flex gap-2">
                                                {isViewingOther ? (
                                                    <>
                                                        ...
                                                        <button className="px-5 md:px-6 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl font-black text-[10px] uppercase tracking-widest text-white transition-all">Message</button>
                                                    </>
                                                ) : (
