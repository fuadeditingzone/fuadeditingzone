                ...
                <div className="bg-[#0a0a0a] border border-white/10 rounded-[2.5rem] p-8 space-y-6 shadow-2xl">
                    <div className="flex gap-4">
                        ...
                        <div className="flex-1 space-y-4">
                            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Project Title" className="w-full bg-black border border-white/5 rounded-xl p-4 text-white text-sm outline-none focus:border-red-600 transition-all" />
                            <textarea value={caption} onChange={(e) => setCaption(e.target.value)} placeholder="Caption & #tags..." className="w-full bg-black border border-white/5 rounded-xl p-4 text-white text-sm outline-none resize-none h-24 focus:border-red-600 transition-all custom-scrollbar" />
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between border-t border-white/5 pt-6">
                        ...
                        <button disabled={isUploading || !selectedFile || !title} onClick={handleUpload} className="bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white px-10 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl transition-all flex items-center gap-2">
                ...
                {posts.map((post) => (
                    <motion.div key={post.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-[#080808] border border-white/5 rounded-[3rem] overflow-hidden shadow-2xl">
                        <div className="p-6 flex items-center justify-between bg-black/20">
                            <div className="flex items-center gap-4 cursor-pointer group" onClick={() => window.history.pushState(null, '', `/@${post.userName}`)}>
                            ...
                            <button onClick={() => copyPostLink(post.id, post.mediaUrl)} className="p-3 rounded-full bg-white/5 text-zinc-500 hover:text-white transition-all border border-transparent hover:border-red-600/30">
                        ...
                        <div className="p-8 space-y-4">
                            ...
                            <p className="text-zinc-400 text-sm leading-relaxed font-medium">{post.caption}</p>
                            {post.tags?.length > 0 && (
