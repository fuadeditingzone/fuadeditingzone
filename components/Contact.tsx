...
export const Contact: React.FC<{ onStartOrder: (platform: 'whatsapp' | 'email') => void }> = ({ onStartOrder }) => {
  ...
  // Initialize EmailJS via Global window object
  useEffect(() => {
    if (window.emailjs) {
      window.emailjs.init("Z-tNNcQWjE-izOeae");
    }
  }, []);

  // 1. Sync User to Firebase for Community Discovery
  useEffect(() => {
    if (isSignedIn && user) {
  ...
  // 2. Real-time Status Monitor + EmailJS Trigger
  useEffect(() => {
    ...
    const unsubscribe = onValue(ordersRef, (snapshot) => {
      ...
      if (data) {
        ...
        // Automation: Detect Status Change
        list.forEach(newOrder => {
          const oldOrder = userOrders.find(o => o.id === newOrder.id);
          
          // Trigger when status moves from Pending to final states
          if (oldOrder && oldOrder.status === 'Pending' && (newOrder.status === 'Accepted' || newOrder.status === 'Rejected')) {
            ...
            if (window.emailjs) {
              ...
              window.emailjs.send("service_default", "template_order_status", emailParams)
                .then(() => console.log("Status Transmission Successful"))
                .catch((err: any) => console.error("Neural Link Comms Failed:", err));
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); // Fixed: Use preventDefault on every submission
    ...
    try {
      ...
      console.error("Critical Signal Failure:", error);
      setStatus('idle');
    }
  };
    ...
    <section ref={intersectionRef} id="contact" className="py-24 bg-black relative z-10 select-none overflow-hidden">
          ...
          >
            <div className="bg-blue-600 text-white p-6 rounded-[2rem] shadow-[0_20px_50px_rgba(37,99,235,0.5)] border border-blue-400/30 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest">Protocol Updated!</p>
              </div>
      ...
      <div className="container mx-auto px-6 max-w-6xl">
        <div className="mb-16 text-center">
          <span className="text-[10px] font-black uppercase tracking-[0.6em] text-red-600 mb-3 block">Neural Terminals</span>
          <h2 className="text-white text-4xl md:text-6xl font-black uppercase tracking-tighter">Initiate Order</h2>
          ...
          <div className="bg-[#080808]/95 border border-white/10 rounded-[3.5rem] p-10 md:p-14 shadow-2xl relative overflow-hidden group">
            {!isSignedIn ? (
              <div className="py-24 text-center space-y-10">
                <div className="w-20 h-20 bg-red-600/5 rounded-full flex items-center justify-center mx-auto border border-red-600/20">
                  <PlayIcon className="w-8 h-8 text-red-600" />
                </div>
                <h3 className="text-2xl font-black text-white uppercase tracking-tight">Identity Synchronization Required</h3>
                <SignInButton mode="modal">
                  <button onClick={(e) => e.preventDefault} className="bg-red-600 hover:bg-red-700 text-white font-black py-5 px-14 uppercase tracking-[0.5em] text-[11px] rounded-2xl transition-all shadow-[0_10px_40px_rgba(220,38,38,0.4)]">Verify Identity</button>
                </SignInButton>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-3">
                    <label className="text-[9px] uppercase font-black text-gray-600 tracking-widest ml-1">Verified Relay</label>
                ...
                <div className="space-y-3">
                  <label className="text-[9px] uppercase font-black text-gray-600 tracking-widest ml-1">Service Directive</label>
                  <select 
                    required 
                    name="service" 
                    value={formData.service} 
                    onChange={e => setFormData(p => ({ ...p, service: e.target.value }))} 
                    className="w-full bg-black border border-blue-600/40 rounded-2xl px-6 py-5 text-sm text-white focus:border-red-600 outline-none transition-all cursor-pointer shadow-lg"
                  >
                ...
                <div className="space-y-3">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-black border border-blue-600/40 rounded-2xl px-6 py-5 text-sm text-white focus:border-red-600 outline-none resize-none transition-all shadow-lg" 
                    placeholder="Enter your mission details..." 
                  />
                </div>
                ...
                >
                  {status === 'submitting' ? <SparklesIcon className="w-5 h-5 animate-spin" /> : 
                   hasPendingOrder ? 'Mission in Progress' : 'Dispatch Order'}
                </button>
                
                {hasPendingOrder && (
                  <p className="text-[9px] text-center text-yellow-500/80 uppercase font-black tracking-widest mt-4 animate-pulse">
                    Please await completion of current mission before new dispatch.
                  </p>
                )}
              </form>
            )}
                ...
                <div className="space-y-6 max-h-[550px] overflow-y-auto pr-3 custom-scrollbar">
                    {userOrders.length === 0 ? (
                        <div className="py-24 text-center opacity-10 flex flex-col items-center gap-6">
                            ...
                            <p className="text-[11px] uppercase font-black tracking-[0.8em]">No Signals Found</p>
                        </div>
                    ) : (
                        userOrders.map((order) => (
                            <motion.div 
                                ...
                                className={`p-8 rounded-[2rem] border transition-all duration-700 ${
                                    order.status === 'Pending' ? 'bg-yellow-600/5 border-yellow-500/20 shadow-[0_0_30px_rgba(234,179,8,0.1)]' :
                                    order.status === 'Accepted' ? 'bg-blue-600/10 border-blue-500/40 shadow-[0_0_35px_rgba(37,99,235,0.15)] status-update' :
                                    order.status === 'Rejected' ? 'bg-red-600/10 border-red-500/30' :
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border flex-shrink-0 ${
                                        order.status === 'Pending' ? 'text-yellow-500 border-yellow-500/30 bg-yellow-500/10' :
                                        order.status === 'Accepted' ? 'text-blue-500 border-blue-500/40 bg-blue-500/20' :
                                        order.status === 'Rejected' ? 'text-red-500 border-red-500/30 bg-red-500/10' :
                                        'text-green-500 border-green-500/30 bg-green-500/10'
                                    }`}>
                                        {order.status}
                                    </div>
                                </div>
                                <p className="text-[11px] text-gray-500 font-medium leading-relaxed italic border-l-2 border-white/5 pl-4 line-clamp-3">
                                  "{order.message}"
                                </p>
                            </motion.div>
                        ))
