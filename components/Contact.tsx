...
interface Order {
  id: string;
  userId: string; // for admin panel tracking
  service: string;
  message: string;
...
export const Contact: React.FC<{ onStartOrder: (platform: 'whatsapp' | 'email') => void }> = ({ onStartOrder }) => {
  ...
  // Initialize EmailJS
  useEffect(() => {
    if (window.emailjs) window.emailjs.init("Z-tNNcQWjE-izOeae");
  }, []);
  ...
  const handleUpdateStatus = async (e: React.MouseEvent, order: Order, newStatus: 'Accepted' | 'Rejected') => {
    e.preventDefault();
    if (!isOwner) return;
    ...
    try {
      ...
      // Trigger EmailJS
      if (window.emailjs && order.userEmail) {
        const emailParams = {
          user_name: order.userName || 'Artist',
          user_email: order.userEmail,
          order_status: newStatus,
          service_type: order.service,
          admin_handle: "@fuadeditingzone"
        };
        window.emailjs.send("service_default", "template_order_status", emailParams)
          .then(() => console.log(`Email Sent: Order ${newStatus}`))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch(siteConfig.api.formspreeEndpoint, {
        ...
        body: JSON.stringify({ ...orderData, _subject: `NEW ORDER: ${formData.service}` })
      });
      setStatus('success');
          ...
          >
            <div className="bg-blue-600 text-white p-6 rounded-[2rem] shadow-[0_20px_50px_rgba(37,99,235,0.5)] border border-blue-400/30 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest">Neural Link Updated!</p>
              </div>
      ...
      <div className="container mx-auto px-6 max-w-6xl">
        <div className="mb-16 text-center">
          ...
          <h2 className="text-white text-4xl md:text-6xl font-black uppercase tracking-tighter">
            {isOwner ? 'Overseer Terminal' : 'Initiate Dispatch'}
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          {/* LEFT: FORM (USER) OR ADMIN LOG (OWNER) */}
            ...
            {isOwner ? (
                ...
                <div className="space-y-8">
                    ...
                    <div className="space-y-4 max-h-[600px] overflow-y-auto pr-3 custom-scrollbar">
                        {adminOrders.length === 0 ? (
                            <p className="text-gray-500 text-[10px] uppercase font-black text-center py-20 tracking-widest">No Client Data Found</p>
                        ) : (
                            adminOrders.map(order => (
                                <div key={order.id} className="p-6 bg-white/5 border border-white/10 rounded-3xl relative overflow-hidden group/order">
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <div className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest border ${
                                            order.status === 'Pending' ? 'text-yellow-500 border-yellow-500/20' : 'text-blue-500 border-blue-500/20'
                                        }`}>{order.status}</div>
                                    </div>
                                    ...
                                    {order.status === 'Pending' && (
                                        <div className="flex gap-2">
                                            <button onClick={(e) => handleUpdateStatus(e, order, 'Accepted')} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-black py-2 rounded-xl text-[9px] uppercase tracking-widest transition-all active:scale-95 shadow-lg">Accept</button>
                                            <button onClick={(e) => handleUpdateStatus(e, order, 'Rejected')} className="flex-1 bg-red-600/10 hover:bg-red-600 text-white border border-red-600/20 font-black py-2 rounded-xl text-[9px] uppercase tracking-widest transition-all active:scale-95">Reject</button>
                                        </div>
                                    )}
                                </div>
                            ))
                ...
                !isSignedIn ? (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-10">
                        ...
                        <div className="space-y-3">
                            <label className="text-[9px] uppercase font-black text-gray-600 tracking-widest ml-1">Protocol Type</label>
                            <select required name="service" value={formData.service} onChange={e => setFormData(p => ({ ...p, service: e.target.value }))} className="w-full bg-black border border-blue-600/40 rounded-2xl px-6 py-5 text-sm text-white focus:border-red-600 outline-none transition-all cursor-pointer shadow-lg">
                                <option value="" disabled>Select Objective...</option>
                                <option value="VFX Mastering">VFX Mastering</option>
                                <option value="Elite Thumbnail Design">Elite Thumbnail Design</option>
                                <option value="Photo Composition">Photo Composition</option>
                                <option value="High-Tier AMV Edit">High-Tier AMV Edit</option>
                            </select>
                        </div>
                        <div className="space-y-3">
                            <label className="text-[9px] uppercase font-black text-gray-600 tracking-widest ml-1">Creative Briefing</label>
                            <textarea required rows={4} value={formData.message} onChange={e => setFormData(p => ({ ...p, message: e.target.value }))} className="w-full bg-black border border-blue-600/40 rounded-2xl px-6 py-5 text-sm text-white focus:border-red-600 outline-none resize-none transition-all shadow-lg" placeholder="Enter your mission details..." />
                        </div>
                        <button type="submit" disabled={status === 'submitting' || hasPendingOrder} className={`w-full py-6 rounded-2xl text-[11px] font-black uppercase tracking-[0.6em] transition-all flex items-center justify-center gap-4 ${hasPendingOrder ? 'bg-yellow-600/10 text-yellow-500 cursor-not-allowed border border-yellow-500/20' : 'bg-red-600 hover:bg-red-700 text-white shadow-xl active:scale-95'}`}>
                            {status === 'submitting' ? <SparklesIcon className="w-5 h-5 animate-spin" /> : hasPendingOrder ? 'Mission in Progress' : 'Dispatch Order'}
                        </button>
                    </form>
                )
            )}
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 gap-6">
                <button onClick={(e) => { e.preventDefault(); onStartOrder('whatsapp'); }} className="p-8 bg-green-600/10 border border-green-600/20 rounded-[3rem] text-center group hover:bg-green-600/20 transition-all shadow-xl active:scale-95"><WhatsAppIcon className="w-8 h-8 text-green-500 mx-auto mb-4 transition-transform group-hover:-translate-y-2" /><p className="text-[10px] font-black text-white uppercase tracking-widest">Neural WhatsApp</p></button>
                <button onClick={(e) => { e.preventDefault(); onStartOrder('email'); }} className="p-8 bg-red-600/10 border border-red-600/20 rounded-[3rem] text-center group hover:bg-red-600/20 transition-all shadow-xl active:scale-95"><EmailIcon className="w-8 h-8 text-red-500 mx-auto mb-4 transition-transform group-hover:-translate-y-2" /><p className="text-[10px] font-black text-white uppercase tracking-widest">Official Comms</p></button>
            </div>
          </div>
        </div>
      </div>
