...
import { EmailIcon, WhatsAppIcon, SparklesIcon, CloseIcon, VfxIcon, ThumbnailIcon, BannerIcon } from './Icons';
...
const SERVICE_TIERS = [
  { id: 'vfx', name: 'Cinematic VFX Mastery', price: '50', icon: VfxIcon },
  { id: 'thumbnail', name: 'High-CTR Thumbnails', price: '15', icon: ThumbnailIcon },
  { id: 'banner', name: 'Elite Branding Kit', price: '25', icon: BannerIcon },
  { id: 'custom', name: 'Custom Request', price: '?', icon: SparklesIcon }
];
    ...
    <section ref={intersectionRef} id="contact" className="py-24 bg-black relative z-10 select-none overflow-hidden">
      <div className="container mx-auto px-6 max-w-6xl">
        <div className="mb-16 text-center">
          ...
          <h2 className="text-white text-4xl md:text-6xl font-black uppercase tracking-tighter">Place Your Order</h2>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-inner relative overflow-hidden flex flex-col h-full min-h-[500px] max-h-[85vh]">
                <h3 className="text-2xl font-black text-white uppercase tracking-widest mb-8 pb-8 border-b border-white/5 flex-shrink-0">My Orders</h3>
                <div className="space-y-6 overflow-y-auto custom-scrollbar flex-1 min-h-0 pr-2">
                    {userOrders.length === 0 ? (
                        <div className="py-24 text-center opacity-10 flex flex-col items-center gap-6">
                            ...
                            <p className="text-[11px] uppercase font-black tracking-widest">No order history recorded</p>
                        </div>
                    ) : (
                        userOrders.map((order, idx) => (
                            <motion.div key={idx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem] shadow-lg group hover:border-red-600/20 transition-all duration-500">
                                ...
                                <div className="grid grid-cols-2 gap-3 opacity-60 group-hover:opacity-100 transition-opacity">
                                    <div className="bg-black/60 p-3 rounded-xl border border-white/5 text-[9px] font-black text-gray-400 uppercase tracking-widest text-center">Budget: <span className="text-white">{order.price}</span></div>
                                    <div className="bg-black/60 p-3 rounded-xl border border-white/5 text-[9px] font-black text-gray-400 uppercase tracking-widest text-center">Time: <span className="text-white">{order.delivery}</span></div>
                                </div>
                            </motion.div>
                        ))
                    )}
